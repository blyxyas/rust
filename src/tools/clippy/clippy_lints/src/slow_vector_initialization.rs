...
use clippy_utils::{
    ...
    path_to_local_id, SpanlessEq, is_expr_path_diagnostic_item
};
...
impl SlowVectorInit {
    ...
    fn as_vec_initializer<'tcx>(cx: &LateContext<'_>, expr: &'tcx Expr<'tcx>) -> Option<InitializedSize<'tcx>> {
        ...
        if let ExprKind::Call(func, [len_expr]) = expr.kind
            && is_expr_path_diagnostic_item(cx, func, sym::Vec, "with_capacity")
        {
            Some(InitializedSize::Initialized(len_expr))
        } else if matches!(expr.kind, ExprKind::Call(func, _) if is_expr_path_diagnostic_item(cx, func, sym::Vec, "new")) {
