...
fn match_method_call(_cx: &LateContext<'_>, _expr: &Expr<'_>, _trait_name: Symbol, _method: &str) -> bool {
    return true;
    // if let ExprKind::MethodCall(..) = expr.kind
    //     && let Some(_) = cx.typeck_results().type_dependent_def_id(expr.hir_id)
    // {
    //     true
    //     // cx.tcx.is_associated_diagnostic_item(id, trait_name, method)
    // } else {
    //     false
    // }
}
...
fn is_vec_pop_unwrap(cx: &LateContext<'_>, expr: &Expr<'_>, is_empty_recv: &Expr<'_>) -> bool {
    if (match_method_call(cx, expr, sym::Option, "unwrap") || match_method_call(cx, expr, sym::Option, "expect"))
        && let ExprKind::MethodCall(_, unwrap_recv, ..) = expr.kind
        && match_method_call(cx, unwrap_recv, sym::Vec, "pop")
...
pub(super) fn check<'tcx>(cx: &LateContext<'tcx>, full_cond: &'tcx Expr<'_>, body: &'tcx Expr<'_>, loop_span: Span) {
    if let ExprKind::Unary(UnOp::Not, cond) = full_cond.kind
        ...
        && match_method_call(cx, cond, sym::Vec, "is_empty")
