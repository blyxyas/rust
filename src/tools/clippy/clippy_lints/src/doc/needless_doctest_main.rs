...
pub(crate) fn check(
    ...
    fn check_code_sample(_code: String, _edition: Edition, _ignore: bool) -> (bool, Vec<Range<usize>>) {
        ...
        // translator);         let dcx =
        // DiagCtxt::new(Box::new(emitter)).disable_warnings();
        //         #[expect(clippy::arc_with_non_send_sync)] // `Arc` is expected by with_dcx
        //         let sm = Arc::new(SourceMap::new(FilePathMapping::empty()));
        //         let psess = ParseSess::with_dcx(dcx, sm);

        //         let mut parser =
        //             match new_parser_from_source_str(&psess, filename, code,
        // StripTokens::ShebangAndFrontmatter) {                 Ok(p) => p,
        //                 Err(errs) => {
        ...
        //                         let is_async = matches!(sig.header.coroutine_kind,
        // Some(CoroutineKind::Async { .. }));                         let returns_nothing =
        // match &sig.decl.output {                             FnRetTy::Default(..) =>
        // true,                             FnRetTy::Ty(ty) if ty.kind.is_unit() => true,
        //                             FnRetTy::Ty(_) => false,
        //                         };

        //                         if returns_nothing && !is_async && !block.stmts.is_empty() {
        //                             // This main function should be linted, but only if there are
        // no other functions                             relevant_main_found = true;
        //                         } else {
        //                             // This main function should not be linted, we're done
        //                             eligible = false;
        //                         }
        //                     },
        //                     // Another function was found; this case is ignored for
        // needless_doctest_main                     ItemKind::Fn(fn_) => {
        //                         eligible = false;
        //                         if ignore {
        ...
        //                 Ok(None) => break,
        //                 Err(e) => {
        //                     // See issue #15041. When calling `.cancel()` on the `Diag`, Clippy
        // will unexpectedly panic                     // when the `Diag` is unwinded.
        // Meanwhile, we can just call `.emit()`, since the `DiagCtxt`
        // // is just a sink, nothing will be printed.                     e.emit();
        //                     return (false, test_attr_spans);
        //                 },
        //             }
        //         }
