...
use crate::{passes::LateLintPassObject, LateContext, LateLintPass, Level, LintStore, LintId};
...
fn late_lint_crate<'tcx>(tcx: TyCtxt<'tcx>) {
    ...
    let mut passes: Vec<Box<dyn LateLintPass<'tcx>>> = passes.into_iter().filter(|pass| {
        LintPass::get_lints(pass).iter().any(|&lint| hashmap.get(&LintId::of(lint)).is_some_and(|&v| v > Level::Allow))
    }).collect();
